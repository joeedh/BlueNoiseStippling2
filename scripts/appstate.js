            ...
            render, exportsvg)
{
  ...
  let AppState = exports.AppState = class {
    ...
    reset() {
      ...
      if (RASTER_IMAGE)  {
        let size = ~~(SCALE*Math.min(canvas.width, canvas.height));
        size = DIMEN;
    ...
    renderImage(canvas, g, drawBG=true) {
      let size = RENDERED_IMAGE_SIZE;
      let oldscale = window.SCALE;
      
      window.SCALE = 1.0;
      
      console.log("rendering image. . .");
      
      let asp = _appstate.image.width / _appstate.image.height;
      let w, h;
    ...
    save() {
      localStorage.startup_file_bn7 = JSON.stringify(this);
    }
    
    load() {
      try {
        this.loadJSON(JSON.parse(localStorage.startup_file_bn7));
      } catch (error) {
    ...
    clearData() {
      ...
      delete localStorage.startup_file_bn7;
    ...
    makeGUIOld() {
      ...
      //they typically take the constant name (converted to lowercase)
      //as their first argument
      let gui2 = this.gui2 = new ui.UI("ui2_bn6", window);
      let gui = this.gui = new ui.UI("ui1_bn6", window);
