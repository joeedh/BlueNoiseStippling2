...
define([
  ...
  let makeUIName = exports.makeUIName = function makeUIName(s) {
    let parts = [''];
    ...
    for (let i = 0; i < s.length; i++) {
      if (s[i] === ' ') {
        parts.push('');
        continue;
      } else if (p !== undefined && isUpper(s[i]) && !isUpper(p)) {
        parts.push('');
      }
    ...
    parts = parts.map(c => c.trim()).filter(c => c.length);
    parts = parts.map(c => c[0].toUpperCase() + c.slice(1, c.length).toLowerCase());
    return parts.join(" ");
  }
  ...
  */
  ...
  var UI = exports.UI = class UI {
    ...
    _checkPath(path, val) {
      if (window._checkConfigKey) {
        window._checkConfigKey(path, val);
      }
    }

    check(path, name) {
      this._checkPath(path, false);
      ...
      if (this._path_get(path)[0] === undefined) {
        this._path_set(path, false);
      }
    ...
    */
    ...
    slider(path, name, defaultval, min, max, step, is_int, do_redraw, callback, tooltip) {
      this._checkPath(path, defaultval||0.0);
