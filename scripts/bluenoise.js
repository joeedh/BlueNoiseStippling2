...
define([
  ...
  function initMessageSystem() {
    ...
    function message(msg) {
      ...
      for (let i = 0; i < lines.length; i++) {
        buffer[buffer.cur] = lines[i];
        buffer.cur = (buffer.cur + 1)%buffer.length;
      }

      let text = "";

      for (let i = 0; i < buffer.length; i++) {
        let i2 = (i + buffer.cur + buffer.length)%buffer.length;
        text += buffer[i2] + "<br>";
      }
  ...
  let {pushMessage, clearPushedMessages} = initMessageSystem();
  ...
  exports.BlueNoise = class BlueNoise {
    constructor() {
      this.done = false;
      ...
      this.load_mask(smoothmask_file);
    }

    step_b(custom_steps, skip_points_display) {
    ...
    step_diffusion(custom_steps, cur) {
      ...
      for (var si = 0; si < steps; si++, cur++) {
        ...
        if (do_raster) {
          ...
          var rix = ~~((x*0.5 + 0.5001)*raster_image.width*0.99999),
    ...
    step_cmyk_color_mask(custom_steps, skip_points_display) {
    ...
    step(custom_steps, skip_points_display) {
      if (this.smask !== undefined) {
        return this.step_smask(custom_steps);
      }
      ...
      if (this.sampler === undefined) {
        this.message("image hasn't loaded yet");
      }

      if (this.mask === undefined) {
        this.message("mask hasn't loaded yet");
        return;
      }
      ...
      if (this.mask.data === undefined) {
        this.message("WARNING: mask failed to load");
        return;
      }
      ...
      for (var si = 0; si < steps; si++, this.cur++) {
        ...
        if (do_raster && RASTER_MODE === RASTER_MODES.DIFFUSION) {
          ...
          var rix2 = ~~((x*0.5 + 0.5001)*2*raster_image.width*0.99999),
              riy2 = ~~(2*(y*0.5 + 0.5001)*raster_image.height*0.99999);
    ...
    init(size) {
      ...
      new indexdb_store.IndexDBStore("bluenoise7_mask").read("data").then((result) => {
        if (result === undefined) {
